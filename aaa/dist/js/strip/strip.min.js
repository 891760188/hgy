...
! function(a) {
		...
		Fit = {
			within: function(a, b) {
				var c = $.extend({
					...
					e = 1,
					f = 5;
				c.border && (b.width -= 2 * c.border, b.height -= 2 * c.border);
				var g = {
					height: !0,
					width: !0
				};
				...
				for(; f > 0 && (g.width && d.width > b.width || g.height && d.height > b.height);) {
					var h = 1,
						i = 1;
					g.width && d.width > b.width && (h = b.width / d.width), g.height && d.height > b.height && (i = b.height / d.height);
					var e = Math.min(h, i);
					d = {
	...
	$.extend(ImageReady.prototype, {
		...
		initialize: function(a, b, c) {
			return this.img = $(a), this.callback = b, this.errorCallback = c, this._supports.naturalWidth ? (this.img.bind("error", $.proxy(this.error, this)), this.intervals = [
				[1e3, 10],
				[2e3, 50],
				[4e3, 100],
				[2e4, 500]
			], this._ipos = 0, this._time = 0, this._delay = this.intervals[this._ipos][1], void this.tick()) : void this.fallback()
		},
		tick: function() {
			this._ticking = setTimeout($.proxy(function() {
				if(this.img[0].naturalWidth > 0) return void this.callback(this.img[0]);
				if(this._time += this._delay, this._time > this.intervals[this._ipos][0]) {
		...
		fallback: function() {
			...
			this._fallbackImg = a, a.onload = $.proxy(function() {
				a.onload = function() {}, this.img[0].naturalWidth = a.width, this.img[0].naturalHeight = a.height, this.callback(this.img[0])
			}, this), a.onerror = $.proxy(this.error, this), a.src = this.img.attr("src")
		},
		abort: function() {
			this._fallbackImg && (this._fallbackImg.onload = function() {}), this._ticking && (clearTimeout(this._ticking), this._ticking = 0)
		},
		error: function() {
			this.errored || (this.errored = !0, this.abort(), this.errorCallback && this.errorCallback())
		}
		...
		start: function() {
			var a = {};
			debugger
			var dom = document.getElementById('id-strp-window');
			a[Support.css.prefixed("animation")] = this.classPrefix + "spinner-spin 0s infinite steps(" + this.lines + ")", this._rotate.css(a)
		},
		stop: function() {
			var a = {};
		...
		show: function(a) {
			this.build(), 
			this.start(), 
			this.element.stop(!0).fadeTo(this.options.show, 0.1, a)
		},
		hide: function(a) {
	...
	var Types = {
		...
		Options = function() {
			function a(a, d, e) {
				a = a || {}, e = e || {}, a.skin = a.skin || Window.defaultSkin;
				var f = a.skin ? $.extend({}, Strip.Skins[a.skin] || Strip.Skins[Window.defaultSkin]) : {},
					g = deepExtendClone(c, f);
				d && g.initialTypeOptions[d] && (g = deepExtendClone(g.initialTypeOptions[d], g), delete g.initialTypeOptions);
				var h = deepExtendClone(g, a);
				return(!h.effects || Browser.IE && Browser.IE < 9) && (h.effects = {}, $.each(b.effects, function(a, b) {
					$.each(h.effects[a] = $.extend({}, b), function(b) {
						h.effects[a][b] = 0
					})
				})), h.keyboard && ("boolean" == $.type(h.keyboard) && (h.keyboard = {}, $.each(b.keyboard, function(a) {
					h.keyboard[a] = !0
				})), ("vimeo" == d || "youtube" == d) && $.extend(h.keyboard, {
					left: !1,
					right: !1
				})), ("vimeo" == d || "youtube" == d) && (h.overlap = !1), h
			}
			var b = Strip.Skins.base,
				c = deepExtendClone(b, Strip.Skins.reset);
			return {
				create: a
			}
		}();
	...
	var Pages = {
			initialize: function(a) {
				this.element = a, this.pages = {}, this.uid = 1, this._loading = {}
			},
			add: function(a) {
				this.uid++, this.views = a, this.pages[this.uid] = [], $.each(a, $.proxy(function(a, b) {
					this.pages[this.uid].push(new Page(b, a + 1, this.views.length))
				}, this)), this._switched = !0
			},
			show: function(a, b) {
				debugger;
				var dom = document.getElementById('id-strp-window');
				...
				return this.page && this.page.uid == c.uid ? void(
					c.view.options.toggle && (Window.hide(), this.page = null)
					) : 
					(this.page = c, this.removeHiddenAndLoadingInactive(), 
					void c.show(
						$.proxy(function() {
...
//			},
		...
		Page = function() {
			...
			return $.extend(b.prototype, {
				initialize: function(b, c, d) {
					this.view = b, this.dimensions = {
						width: 0,
						height: 0
					}, this.uid = a++, this._position = c, this._total = d, this.animated = !1, this.visible = !1, this.queues = {}, this.queues.showhide = $({})
				},
				create: function() {
					if(!this._created) {
				...
				load: function(a) {
					...
					switch(this.abort(), this.loading = !0, Window.startLoading(), this.view.type) {
						case "image":
				...
				_markAsLoaded: function() {
					this.loading = !1, this.loaded = !0, Window.stopLoading()
				},
					...
					switch(this.view.type) {
						case "vimeo":
							...
							this.content.append(this.playerIframe = $("<iframe webkitAllowFullScreen mozallowfullscreen allowFullScreen>").attr({
								src: "//player.vimeo.com/video/" + this.view._data.id + "?" + c,
							...
							break;
						case "youtube":
							var b = this.view.options.youtube || {},
								c = $.param(b);
							this.content.append(this.playerIframe = $("<iframe webkitAllowFullScreen mozallowfullscreen allowFullScreen>").attr({
								src: "//www.youtube.com/embed/" + this.view._data.id + "?" + c,
								height: this.contentDimensions.height,
								width: this.contentDimensions.width,
								frameborder: 0
							})), a && a()
				...
				show: function(a) {
					...
					b.queue([]), this.animated = !0, this.animatingWindow = !1, b.queue(function(a) {
						...
						Window._spinner && (Window.setSpinnerSkin(this.view.options.skin), Window._spinner.setOptions(this.view.options.effects.spinner), Window._spinner.refresh()), this.load($.proxy(function() {
					...
					}, this)), ("vimeo" == this.view.type || "youtube" == this.view.type) && b.queue($.proxy(function(a) {
						...
						var b = 3,
							...
							d = Window.resize(this[c], function() {
								--b < 1 && a()
							}, d);
						this._show(function() {
							--b < 1 && a()
				...
				_show: function(a, b) {
					...
					this.element.stop(!0).show().fadeTo(c || 0, 1, "swing", a)
				},
				hide: function(a, b) {
					if(this.element) {
						...
						"number" == $.type(b) && (c = b);
						var d = "youtube" == this.view.type || "vimeo" == this.view.type;
						d && (c = 0), this.element.stop(!0).fadeTo(c, 0, "easeInCubic", $.proxy(function() {
				...
				removeVideo: function() {
					if(this.playerIframe && (this.playerIframe[0].src = "//about:blank", this.playerIframe.remove(), this.player_frame = null), this.player) {
						try {
							this.player.destroy()
						} catch(a) {}
						this.player = null
					}
					this.playerDiv && (this.playerDiv.remove(), this.playerDiv = null)
				},
				remove: function() {
				...
				_getDimensionsFitToView: function() {
					var a = $.extend({}, this.dimensions),
						...
						c = this.view.options;
					return c.maxWidth && (a.width = c.maxWidth), c.maxHeight && (a.heigth = c.maxHeight), b = Fit.within(b, a)
				},
				...
				fitToWindow: function() {
					...
					var h = parseInt(Window.element.css("margin-" + ("width" == f ? "left" : "bottom")));
					Window.element.addClass("strp-measured"), d[f] -= h;
					var i = parseInt(g.css("padding-left")) + parseInt(g.css("padding-right")),
						j = parseInt(g.css("padding-top")) + parseInt(g.css("padding-bottom"));
					d.width -= i, d.height -= j;
					var k = Fit.within(b, d),
						l = $.extend({}, k),
						m = this.content;
					this.error && (m = this.error);
					var n = this.info,
						o = 0;
					if(n) {
						var p = Window.element.is(":visible");
						p || Window.element.show();
						var q = a.is(":visible");
						if(q || a.show(), "width" == f) {
							a.css({
								width: k.width + i + "px"
							...
							var r = d.height;
							m.hide(), o = n.outerHeight(), m.show(), d.height = r - o, l = Fit.within(b, d);
							for(var s, t, u, v = $.extend({}, l), w = o, x = 4; x > 0 && (u = k.width - l.width);) a.css({
								width: k.width + i - u + "px"
							}), t = o, m.hide(), o = n.outerHeight(), s = Math.max(this.caption ? this.caption.outerWidth() + i : 0, this.position ? this.position.outerWidth() + i : 0), m.show(), o == t && s <= k.width + i - u ? k.width -= u : (d.height = r - o, l = Fit.within(b, d), 0 >= x - 1 && (a.css({
								width: k.width + i + "px"
							}), l = v, o = w)), x--
						} else m.hide(), o = n.outerHeight(), m.show(), d.height -= o, l = Fit.within(b, d), k.height = l.height;
						q || a.hide(), p || Window.element.hide()
					}
					a.css("width" == f ? {
						width: k.width + i + "px"
					} : {
						height: k.height + j + o + "px"
					}), g.css({
						bottom: o + "px"
					}), m.css(px($.extend({}, l, {
						"margin-left": Math.floor(-.5 * l.width),
						"margin-top": Math.floor(-.5 * l.height)
					}))), this.player ? this.player.setSize(l.width, l.height) : this.playerIframe && this.playerIframe.attr(l), this.contentDimensions = l, this.width = k.width + i, this.height = k.height + j + o, this.z = this[f]
				}
			}), b
		}(),
		Window = {
			defaultSkin: "strip",
			initialize: function() {
				this.queues = [], this.queues.showhide = $({}), this.pages = [], this.timers = new Timers, this.build(), this.setSkin(this.defaultSkin)
			},
			build: function() {
				debugger
				/**
				...
				Spinner.supported && ($(document.body).append(this.spinnerMove = $("<div>").addClass("strp-spinner-move").hide()
				.append(this.spinner = $("<div>").addClass("strp-spinner"))),
				//hgy
				//this._spinner = new Spinner(this.spinner), this._spinnerMoveSkinless = this.spinnerMove[0].className), $(document.body).append(this.element = $("<div>").addClass("strp-window strp-measured").append(this._pages = $("<div>").addClass("strp-pages")).append(this._previous = $("<div>").addClass("strp-nav strp-nav-previous").append($("<div>").addClass("strp-nav-button").append($("<div>").addClass("strp-nav-button-background")).append($("<div>").addClass("strp-nav-button-icon"))).hide()).append(this._next = $("<div>").addClass("strp-nav strp-nav-next").append($("<div>").addClass("strp-nav-button").append($("<div>").addClass("strp-nav-button-background")).append($("<div>").addClass("strp-nav-button-icon"))).hide()).append(this._close = $("<div>").addClass("strp-close").append($("<div>").addClass("strp-close-background")).append($("<div>").addClass("strp-close-icon")))), Pages.initialize(this._pages), Support.mobileTouch && this.element.addClass("strp-mobile-touch"), Support.svg || this.element.addClass("strp-no-svg"), this._close.on("click", $.proxy(function(a) {
				this._spinner = new Spinner(this.spinner), this._spinnerMoveSkinless = this.spinnerMove[0].className), $(document.body).append(this.element = $("<div>").attr('id','id-strp-window').addClass("strp-window strp-measured").append(this._pages = $("<div>").addClass("strp-pages")).append(this._previous = $("<div>").addClass("strp-nav strp-nav-previous").append($("<div>").addClass("strp-nav-button").append($("<div>").addClass("strp-nav-button-background")).append($("<div>").addClass("strp-nav-button-icon"))).hide()).append(this._next = $("<div>").addClass("strp-nav strp-nav-next").append($("<div>").addClass("strp-nav-button").append($("<div>").addClass("strp-nav-button-background")).append($("<div>").addClass("strp-nav-button-icon"))).hide()).append(this._close = $("<div>").addClass("strp-close").append($("<div>").addClass("strp-close-background")).append($("<div>").addClass("strp-close-icon")))), Pages.initialize(this._pages), Support.mobileTouch && this.element.addClass("strp-mobile-touch"), Support.svg || this.element.addClass("strp-no-svg"), this._close.on("click", $.proxy(function(a) {
			...
			resize: function(a, b, c) {
				...
				if(0 == i) h = this.view.options.effects.window.show, this.element.addClass("strp-opening"), this.opening = !0;
					...
					var j = this.view.options.effects.transition,
						k = j.min,
						l = j.max,
						m = l - k,
						n = Bounds.viewport(),
						...
						p = Math.min(1, o / n[f]);
					h = Math.round(k + p * m)
				}(0 == i || Pages._switched) && this.showUI(null, h), 0 == a && (this.closing = !0, this.element.is(":animated") || this.element.addClass("strp-closing"));
				var q = {
					overflow: "visible"
				};
				q[f] = a + "px";
				...
				var s = this.view.options.onResize,
					t = "function" == $.type(s);
			...
			load: function(a, b) {
				...
				this.views = a, Pages.add(a), b && this.setPosition(b, $.proxy(function() {
					this._loading && c.callback && c.callback()
				}, this))
			},
			setSide: function(a, b) {
				return this.side == a ? void(b && b()) : void(this.visible ? (this.unbindUI(), this.resize(0, $.proxy(function() {
					this._safeResetsAfterSwitchSide(), Pages.hideVisibleInactive(0), this._setSide(a, b)
				}, this))) : this._setSide(a, b))
			},
			...
			setPosition: function(a, b) {
				this._position = a, this.view = this.views[a - 1], this.page = Pages.show(a, $.proxy(function() {
			...
			bindUI: function() {
				...
				this._pages.on("mousemove", ".strp-container", this._onMouseMoveHandler = $.proxy(this._onMouseMove, this)).on("mouseleave", ".strp-container", 
				this._onMouseLeaveHandler = $.proxy(this._onMouseLeave, this)).on("mouseenter", ".strp-container", this._onMouseEnterHandler = $.proxy(this._onMouseEnter, this)),
				 ...
				 * update by hgy
				 */
				//this._pages.on("click", ".strp-container", this._onClickHandler = $.proxy(this._onClick, this))//这个事件去了即可取消点击消失的效果
				this._pages.on("click", ".strp-container", function(e){
					debugger
					//var dom = document.getElementById('id-strp-window');
					//dom.style.display = 'none';
					//Pages.page.hide();
			...
			showUI: function(a, b) {
				...
				"number" == $.type(b) && (d = b), c.stop(!0).fadeTo(d, 1, $.proxy(function() {
			...
			hideUI: function(a, b) {
				...
				"number" == $.type(b) && (d = b), c.stop(!0).fadeOut(d, function() {
					"function" == $.type(a) && a()
				})
			},
			...
			startUITimer: function() {
				this.clearUITimer(), this.timers.set("ui", $.proxy(function() {
					this.hideUI()
				}, this), this.view ? this.view.options.uiDelay : 0)
			}
		};
	Support.mobileTouch && $.each("showUI hideUI clearUITimer startUITimer".split(" "), function(a, b) {
		Window[b] = function() {}
	});
	var Keyboard = {
			enabled: !1,
		...
		_Strip = {
			...
			startDelegating: function() {
				this.stopDelegating(), $(document.documentElement).delegate(".strip[href]", "click", this._delegateHandler = $.proxy(this.delegate, this))
			},
			stopDelegating: function() {
				this._delegateHandler && ($(document.documentElement).undelegate(".strip[href]", "click", this._delegateHandler), this._delegateHandler = null)
			},
			delegate: function(a) {
				if(!this._disabled) {
				...
				switch(object_type = $.type(object)) {
					...
					case "object":
						...
						if(view.group) {
							...
							var groupOptions = {};
							_.isElement(object) && $(object).is("[" + _dgo + "]") && ($.extend(groupOptions, eval("({" + ($(object).attr(_dgo) || "") + "})")), view = new View(object, $.extend({}, groupOptions, options))), views.push(view)
						}
						break;
					case "array":
	...
	}), (Browser.IE && Browser.IE < 7 || "number" == $.type(Browser.Android) && Browser.Android < 3 || Browser.MobileSafari && "number" == $.type(Browser.WebKit) && Browser.WebKit < 533.18) && (_Strip.show = _Strip.showFallback), $(document).ready(function() {
		_Strip.initialize()
	}), Strip
});
