...
(function(){
        ...
        H5lock.prototype.drawCle = function(x, y) { // 初始化解锁密码面板
            this.ctx.strokeStyle = '#CFE6FF';//圆圈周的颜色，和背景想衬托，像是白色
            this.ctx.lineWidth = 2;//院周的厚度
            this.ctx.beginPath();
        ...
        H5lock.prototype.drawPoint = function() { // 初始化圆心
            for (var i = 0 ; i < this.lastPoint.length ; i++) {
                this.ctx.fillStyle = '#CFE6FF';//滑动线的颜色
                this.ctx.beginPath();
        ...
        H5lock.prototype.createCircle = function() {// 创建解锁点的坐标，根据canvas的大小来平均分配半径
            ...
            this.ctx.clearRect(0, 0, this.ctx.canvas.width, this.ctx.canvas.height);//画布画出来
        ...
        H5lock.prototype.bindEvent = function() {
            var self = this;
            this.canvas.addEventListener("touchstart", function (e) {//用户将某个触摸点置于触摸界面上事件
