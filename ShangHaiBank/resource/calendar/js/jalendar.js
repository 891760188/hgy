...
$(function () {    
    (function ($) {
        $.fn.jalendar = function (options) {
            ...
            var lTotalEvents = {};
            var lEvent = {};
            dayNames['ZH-CN'] = new Array('一', '二', '三', '四', '五', '六', '日');
            ...
            monthNames['ZH-CN'] = new Array('1月', '2月', '3月', '4月', '5月', '6月', '7月', '8月', '9月', '10月', '11月', '12月'); 
...
//                      div('div', 'total-bar').html( lTotalEvents[settings.lang] + '<b style="color: '+settings.color+'"></b>'),
            ...
            $this.on('tap', '.this-month', function () {//修改为tap，click在ios上不生效
                var eventSingle = $(this).find('.event-single')
                $this.find('.events .event-single').remove();//清除某天没事件的事件式样
                prevAddEvent();//选中某天的式样
                $(this).addClass('selected').css({'background-color': settings.color});//被点击的当天式样
                //调用具体添加事件
//              goAddEventPage();//跳转到事件添加的界面                       2017年2月13日16:10:59 注释掉，因为在日历中点击某一天会转跳到新增，目前测试没有影响到其他功能
//              var eventSingle = $(this).find('.event-single')//某天没有事件，清除式样
