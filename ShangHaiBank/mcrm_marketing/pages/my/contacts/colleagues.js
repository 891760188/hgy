...
//			dataType: "json",
			success: function(res) {
				...
				for(var i = 0; i < data.length; i++) {
					...
					_html = '<div class="ys-accNode talkDiv"  data-fn=' + dataFun + '><div class="ys-anContent"><b>';
...
//		dataType: 'json',
		success: function(res) {
			...
			for(var i = 0; i < data.length; i++) {
				...
				_html += '<div class="ys-accNode talkDiv" data-fn=' + dataFun + '><div class="ys-anContent">';
				_html += '<b><img src="../../../themes/default/images/temp/' + data[i].img + '" alt="" />';
				_html += '</b>' + data[i].name + '<i>' + data[i].position + '</i>';
				_html += _span;
				_html += '</div></div>';
			}
			_html += '<a href="tel:18030878861" id="telId"></a>';
...
//			dataType: 'json',
			success: function(res) {
//				console.log('请求index');
				var data = indexDatas.data;
				var _html = '<div class="mui-indexed-list-empty-alert">没有数据</div>';
				_html += '<ul class="mui-table-view" style="margin-bottom:40px">';
				for(var i = 0; i < data.length; i++) {
					var dataGroupV = '';
					...
					if(lists.length > 0) {
						...
						for(var j = 0; j < lists.length; j++) { //数据循环
							...
							_html += 'class="mui-table-view-cell mui-indexed-list-item">' + lists[j].name + _span + '</li>'
						}
					}
...
//		dataType: "json",
		success: function(res) {
			...
			for(var i = 0; i < data.length; i++) {
				var _html = '';
				_html = '<div class="ys-accNode talkDiv"  data-fn="makeTalk("' + data[i].makeTalk + '");"><div class="ys-anContent"><b>';
				_html += '<img src="../../../themes/default/images/temp/' + data[i].img + '" alt="" /></b>' + data[i].name + '<i>' + data[i].position + '</i>';
...
//		dataType: 'json',
		...
		success: function(res) {
			var data = commonS.data;
			var _html = '';
			_html = '<div class="ys-accContent">';
			for(var i = 0; i < data.length; i++) {

				_html += '<div class="ys-accNode talkDiv"  data-fn="makeTalk("' + data[i].makeTalk + '");"><div class="ys-anContent">';
				_html += '<b><img src="../../../themes/default/images/temp/' + data[i].img + '" alt="" />';
...
function makeTalk (toUserName) {
	var accountName =  _App.sessionUser.userId();
	if(toUserName==accountName){
		mui.alert('您不能和自己聊天','提示');
	}else{
		ws=plus.nativeUI.showWaiting();
		//先判断是否存在群聊聊天组
		_App.ajax({
		type: "get",
		url: basePath + 'chat!isExistoneChatRoom.json',
		data: {
			chatNameId: toUserName,
			roomtype: '1',
			createpeople: accountName
		},
		cache: false,
		dataType: "json",
		success: function(response) {
			//这里判断是否已经有A和B的聊天组，如果有套用以前的聊天组，没有创建新的聊天组
			
				if (response.json.data.length == 1) {
						var s = response.json.data;
						mui.openWindow({
							url: 'newChat.html',
							id: 'newChat-id',
							extras: {
								chatRoomId: s[0].ROOMID,
								chatRoomName: s[0].ROOMNAME,
								type: s[0].ROOMTYPE,
								createpeople: s[0].CREATEPEOPLE
							}
						});
						ws.close();
				}else{
...
//							//创建聊天组后，需要添加人员
//							if (response) {
//								addChatRoomMembers(response.json, toUserName);
//							} else {
//								mui.alert("创建群组失败!");
//								ws.close();
//							}
