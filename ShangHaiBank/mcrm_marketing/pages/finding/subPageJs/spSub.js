var spConfig = {
	url: basePath + 'ocrmShareAction!getLinkShare.json?bizType=SP', //查询URL
	pageSize: 6, //分页大小
	scrollerId: 'item8mobile', //mui滚动区域div的ID 
	...
	success: function(response) {
		if(response.json.data.length > 0) {
			...
			var _div = document.getElementById('ulId8');
			shareIdList = '';
			...
			for(var i = 0; i < data.length; i++) {
				...
				_html += '<span class="icon-line-82" id="comm_' + tmp.SHARE_ID + '" data-count="' + tmp.COMCOUNT + '"  data-fn="openComment(\'SP\',' + tmp.SHARE_ID + ')"></span>';
				_html += '<span class="' + _zIcon + '" id="sup_' + tmp.SHARE_ID + '" data-issup="' + is_sup + '"  data-fn="doSupport(\'SP\',' + tmp.SHARE_ID + ')"></span></div></div>';
