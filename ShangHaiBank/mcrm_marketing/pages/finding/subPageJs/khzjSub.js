var khzjConfig = {
	// item3mobile
	url: basePath + 'ocrmShareAction!getLinkShare.json?bizType=KHZJ', //查询URL
	pageSize: 6, //分页大小
	scrollerId: 'item13mobile', //mui滚动区域div的ID 
	...
	success: function(response) {
		if(response.json.data.length > 0) {
			...
			var _div = document.getElementById('ulId13');
			shareIdList = '';
			...
			for(var i = 0; i < data.length; i++) {
				...
				if(tmp.SHARE_LINK_ADDR != '') { //判断分享的是链接还是图片
					...
					shareOrLj = '<div><img data-src="' + tmp.SHARE_URL + '" src="loading.png" onload="lazyLoad(this)" style="width:80%;float:left;margin-left:10%;"/></div>';
				}
				...
				_html += '<span class="icon-line-82" id="comm_' + tmp.SHARE_ID + '" data-count="' + tmp.COMCOUNT + '"  data-fn="openComment(\'SCENE\',' + tmp.SHARE_ID + ')"></span>';
				_html += '<span class="' + _zIcon + '" id="sup_' + tmp.SHARE_ID + '" data-issup="' + is_sup + '"  data-fn="doSupport(\'SCENE\',' + tmp.SHARE_ID + ')"></span></div></div>';
