var LogConfig = { //公告配置
	url: basePath + 'ocrmShareAction!getShareByType.json?bizType=LOG', //查询URL
	pageSize: 6, //分页大小  
	async: false,
	...
	success: function(response) {
		if(response.json.data.length > 0) {
			...
			var _div = document.getElementById('ulId2');
			...
			for(var i = 0; i < data.length; i++) {
				...
				var bizCont = '';
				var tmp = data[i];
				if(tmp.LOG_CONTENT.length > '160') {
					bizCont = tmp.LOG_CONTENT.substring(0, 160) + '......';
					disp = "block";
				} else {
					bizCont = tmp.LOG_CONTENT;
					disp = "none";
				}
				var _html = "";
				...
				_html += '<div id="GGCont_' + tmp.SHARE_ID + '" class="contentInfo">' + bizCont + ''
				_html += '</div>'
				_html += '<div class="bottomBtn moreContent" data-fn="Log" GgCont="' + tmp.LOG_CONTENT + '" GgSmallCont="' + bizCont + '" style="display:' + disp + ';">显示更多</div>'
				...
				_html += '<span class="icon-line-82" id="comm_' + tmp.SHARE_ID + '" data-count="' + tmp.COMCOUNT + '"  data-fn="openComment(\'LOG\',' + tmp.SHARE_ID + ')"></span>';
				_html += '<span class="' + _zIcon + '" id="sup_' + tmp.SHARE_ID + '" data-issup="' + is_sup + '" shareName="' + tmp.SHARE_NAME + '" shareId="' + tmp.SHARE_ID + '" data-fn="doSupport(\'LOG\',' + tmp.SHARE_ID + ')" sign="sup">' + _z + '</span>' //点赞
				_html += '</div>'
				_html += '</div>'
