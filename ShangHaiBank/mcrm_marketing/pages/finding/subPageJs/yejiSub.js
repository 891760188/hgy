var yejiConfig = {
	// item10mobile
	url: basePath + 'ocrmShareAction!getLinkShare.json?bizType=YEJI', //查询URL
	pageSize: 6, //分页大小
	scrollerId: 'item10mobile', //mui滚动区域div的ID 
	...
	success: function(response) {
		if(response.json.data.length > 0) {
			...
			var _div = document.getElementById('ulId10');
			shareIdList = '';
			...
			for(var i = 0; i < data.length; i++) {
				...
				_html += '<p></p>';
				_html += '<div><img data-src="' + tmp.SHARE_URL + '" src="photo1.png" onload="lazyLoad(this)" style="width:80%;float:left;margin-left:10%;"/></div>'
				_html += '<div class="find-button find-bt">';
				_html += '<span class="icon-line-101"  data-fn="delShare(' + tmp.SHARE_ID + ')" ></span>';
				_html += '<span class="icon-line-82" id="comm_' + tmp.SHARE_ID + '" data-count="' + tmp.COMCOUNT + '"  data-fn="openComment(\'YEJI\',' + tmp.SHARE_ID + ')"></span>';
				_html += '<span class="' + _zIcon + '" id="sup_' + tmp.SHARE_ID + '" data-issup="' + is_sup + '"  data-fn="doSupport(\'YEJI\',' + tmp.SHARE_ID + ')"></span></div></div>';
