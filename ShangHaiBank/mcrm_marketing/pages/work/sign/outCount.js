...
var addr;//地址
var isLocate=false;
var condition = {inOrOut :" not null"};//查询条件
var searchCount = 1;
var dataLength;//第一次查询数据条数
var appConfig = {
	//是否启用右滑关闭功能
	swipeBack: false, 
	//分页配置 
	pageInfoConfig: {
		url: basePath + 'signCountAction.json?conditions=' + JSON.stringify(condition),
		pageSize: 6,							 //分页大小
...
//					}
			...
			if(searchCount == 1){
				dataLength = table.children.length;
				searchCount++;
			}else{
				searchCount++;
			}
			if(table.children.length > dataLength){
				mapState=1;
			}
			maps.evalJS('deal('+toData+','+longitude+','+latitude+','+mapState+')');
		},
